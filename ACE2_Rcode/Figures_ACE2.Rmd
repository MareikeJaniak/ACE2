...
```{r}
RAxML_tree_rooted <- reroot(RAxML_tree, 39, position = 0.5*RAxML_tree$edge.length[which(RAxML_tree$edge[,2]==39)])
```
...
```{r}
MrBayes_tree_rooted <- reroot(MrBayes_tree, 40, position = 0.5*MrBayes_tree$edge.length[which(MrBayes_tree$edge[,2]==40)])
```
